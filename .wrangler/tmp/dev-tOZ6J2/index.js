...
// src/index.ts
var DGIdbMCP = class extends McpAgent {
  constructor() {
    super(...arguments);
    ...
    this.DGIDB_GRAPHQL_ENDPOINT = "https://dgidb.org/api/graphql";
  }
  static {
    __name(this, "DGIdbMCP");
  }
  async init() {
    console.error("DGIdb MCP Server initialized.");
    this.server.tool(
      "dgidb_graphql_query",
      `Executes a GraphQL query against the Drug Gene Interaction Database (DGIdb) API (https://dgidb.org/api/graphql). DGIdb consolidates information on drug-gene interactions and gene druggability from over 40 sources. Use this tool to:
...
Use GraphQL introspection for schema discovery: '{ __schema { queryType { name } types { name kind description fields { name args { name type { name ofType { name } } } } } } }'. Refer to the DGIdb API documentation (schema provided via GraphiQL at the endpoint) for more details. If a query fails, check the syntax and retry with introspection.`,
      {
        query: z.string().describe(
          `The GraphQL query string to execute against the DGIdb API. Example: '{ genes(names: ["BRAF"]) { nodes { interactions { drug { name conceptId } } } } }'. Use introspection queries like '{ __schema { queryType { name } types { name kind } } }' to discover the schema.`
        ),
        variables: z.record(z.any()).optional().describe(
          `Optional dictionary of variables for the GraphQL query. Example: { "geneNamesArray": ["BRAF", "EGFR"] } to be used in a query like 'query($geneNamesArray: [String!]) { genes(names: $geneNamesArray) { ... } }'.`
        )
      },
  ...
  async executeDGIdbGraphQLQuery(query, variables) {
    try {
      const headers = {
      ...
      console.error(`Client-side error during DGIdb GraphQL request: ${error3 instanceof Error ? error3.message : String(error3)}`);
      ...
      return {
        errors: [{
          ...
          extensions: {
            clientError: true
            // Custom extension to indicate client-side nature of the error
          }
        }]
      };
    }
...
var src_default = {
  async fetch(request, env2, ctx) {
    ...
    if (url.pathname === "/sse" || url.pathname.startsWith("/sse/")) {
      return DGIdbMCP.serveSSE("/sse").fetch(request, env2, ctx);
    }
...
// .wrangler/tmp/bundle-zC1K2Z/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-zC1K2Z/middleware-loader.entry.ts
